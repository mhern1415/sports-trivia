...
import { BasketballMediumData } from "../../QuizData/Basketball/BasketballMediumData";
import ProgressBar from 'react-bootstrap/ProgressBar'


class BasketballMediumQuiz extends React.Component {
  state = {
    currentQuestion: 0,
    myAnswer: null,
    options: [],
  ...
  loadQuizData = () => {
    this.setState(() => {
      return {
        questions: BasketballMediumData[this.state.currentQuestion].question,
        answer: BasketballMediumData[this.state.currentQuestion].answer,
        options: BasketballMediumData[this.state.currentQuestion].options
      };
    });
  };
  ...
  finishHandler = () => {
    if (this.state.currentQuestion === BasketballMediumData.length - 1) {
      this.setState({
        isEnd: true
      });
    }
    ...
    if (isEnd) {
      ...
      <div className="gif-container">
        ...
        <div style={{ display: (showing ? 'block' : 'none') }}>
          ...
          <ul>
            {BasketballMediumData.map((item, index) => (
...
<span>Try Again!</span> </button> 
      ...
      return (
        ...
        <div className="App">
          ...
          <span>{`Question ${currentQuestion + 1} of ${BasketballMediumData.length}`}</span>
          {options.map(option => (
            <p
              key={option.id}
         ...
         `}
          ...
          {currentQuestion < BasketballMediumData.length - 1 && (
            <button
...
export default BasketballMediumQuiz;
