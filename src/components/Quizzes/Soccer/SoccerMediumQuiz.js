...
import { SoccerMediumData } from "../../QuizData/Soccer/SoccerMediumData";
import ProgressBar from 'react-bootstrap/ProgressBar'


class SoccerMediumQuiz extends React.Component {
  state = {
    currentQuestion: 0,
    myAnswer: null,
    options: [],
  ...
  loadQuizData = () => {
    this.setState(() => {
      return {
        questions: SoccerMediumData[this.state.currentQuestion].question,
        answer: SoccerMediumData[this.state.currentQuestion].answer,
        options: SoccerMediumData[this.state.currentQuestion].options
      };
    });
  };
  ...
  finishHandler = () => {
    if (this.state.currentQuestion === SoccerMediumData.length - 1) {
      this.setState({
        isEnd: true
      });
    }
    ...
    if (isEnd) {
      ...
      <div className="gif-container">
        ...
        <div style={{ display: (showing ? 'block' : 'none') }}>
          ...
          <ul>
            {SoccerMediumData.map((item, index) => (
...
<span>Try Again!</span> </button> 
      ...
      return (
        ...
        <div className="App">
          ...
          <span>{`Question ${currentQuestion + 1} of ${SoccerMediumData.length}`}</span>
          {options.map(option => (
            <p
              key={option.id}
         ...
         `}
          ...
          {currentQuestion < SoccerMediumData.length - 1 && (
            <button
...
export default SoccerMediumQuiz;
