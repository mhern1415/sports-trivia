...
import { SoccerHardData } from "../../QuizData/Soccer/SoccerHardData";
import ProgressBar from 'react-bootstrap/ProgressBar'


class SoccerHardQuiz extends React.Component {
  state = {
    currentQuestion: 0,
    myAnswer: null,
    options: [],
  ...
  loadQuizData = () => {
    this.setState(() => {
      return {
        questions: SoccerHardData[this.state.currentQuestion].question,
        answer: SoccerHardData[this.state.currentQuestion].answer,
        options: SoccerHardData[this.state.currentQuestion].options
      };
    });
  };
  ...
  finishHandler = () => {
    if (this.state.currentQuestion === SoccerHardData.length - 1) {
      this.setState({
        isEnd: true
      });
    }
...
<span>Try Again!</span> </button> 
      ...
      return (
        ...
        <div className="App">
          ...
          <span>{`Question ${currentQuestion + 1} of ${SoccerHardData.length}`}</span>
          {options.map(option => (
            <p
              key={option.id}
         ...
         `}
          ...
          {currentQuestion < SoccerHardData.length - 1 && (
            <button
