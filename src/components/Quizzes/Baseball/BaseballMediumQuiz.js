...
import { BaseballMediumData } from "../../QuizData/Baseball/BaseballMediumData";
import ProgressBar from 'react-bootstrap/ProgressBar'


class BaseballMediumQuiz extends React.Component {
  state = {
    currentQuestion: 0,
    myAnswer: null,
    options: [],
  ...
  loadQuizData = () => {
    this.setState(() => {
      return {
        questions: BaseballMediumData[this.state.currentQuestion].question,
        answer: BaseballMediumData[this.state.currentQuestion].answer,
        options: BaseballMediumData[this.state.currentQuestion].options
      };
    });
  };
  ...
  finishHandler = () => {
    if (this.state.currentQuestion === BaseballMediumData.length - 1) {
      this.setState({
        isEnd: true
      });
    }
    ...
    if (isEnd) {
      ...
      <div className="gif-container">
        ...
        <div style={{ display: (showing ? 'block' : 'none') }}>
          ...
          <ul>
            {BaseballMediumData.map((item, index) => (
...
<span>Try Again!</span> </button> 
      ...
      return (
        ...
        <div className="App">
          ...
          <span>{`Question ${currentQuestion + 1} of ${BaseballMediumData.length}`}</span>
          {options.map(option => (
            <p
              key={option.id}
         ...
         `}
          ...
          {currentQuestion < BaseballMediumData.length - 1 && (
            <button
...
export default BaseballMediumQuiz;
