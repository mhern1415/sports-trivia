...
import { FootballEasyData } from "../../QuizData/Football/FootballEasyData";
import ProgressBar from 'react-bootstrap/ProgressBar'


class FootballEasyQuiz extends React.Component {
  state = {
    currentQuestion: 0,
    myAnswer: null,
    options: [],
  ...
  loadQuizData = () => {
    this.setState(() => {
      return {
        questions: FootballEasyData[this.state.currentQuestion].question,
        answer: FootballEasyData[this.state.currentQuestion].answer,
        options: FootballEasyData[this.state.currentQuestion].options
      };
    });
  };
  ...
  finishHandler = () => {
    if (this.state.currentQuestion === FootballEasyData.length - 1) {
      this.setState({
        isEnd: true
      });
    }
    ...
    if (isEnd) {
      ...
      <div className="gif-container">
        ...
        <div style={{ display: (showing ? 'block' : 'none') }}>
          ...
          <ul>
            {FootballEasyData.map((item, index) => (
...
<span>Try Again!</span> </button> 
      ...
      return (
        ...
        <div className="App">
          ...
          <span>{`Question ${currentQuestion + 1} of ${FootballEasyData.length}`}</span>
          {options.map(option => (
            <p
              key={option.id}
         ...
         `}
          ...
          {currentQuestion < FootballEasyData.length - 1 && (
            <button
...
export default FootballEasyQuiz;
