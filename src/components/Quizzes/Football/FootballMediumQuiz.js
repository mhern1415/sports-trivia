...
import { FootballMediumData } from "../../QuizData/Football/FootballMediumData";
import ProgressBar from 'react-bootstrap/ProgressBar'


class FootballMediumQuiz extends React.Component {
  state = {
    currentQuestion: 0,
    myAnswer: null,
    options: [],
  ...
  loadQuizData = () => {
    this.setState(() => {
      return {
        questions: FootballMediumData[this.state.currentQuestion].question,
        answer: FootballMediumData[this.state.currentQuestion].answer,
        options: FootballMediumData[this.state.currentQuestion].options
      };
    });
  };
  ...
  finishHandler = () => {
    if (this.state.currentQuestion === FootballMediumData.length - 1) {
      this.setState({
        isEnd: true
      });
    }
    ...
    if (isEnd) {
      ...
      <div className="gif-container">
        ...
        <div style={{ display: (showing ? 'block' : 'none') }}>
          ...
          <ul>
            {FootballMediumData.map((item, index) => (
...
<span>Try Again!</span> </button> 
      ...
      return (
        ...
        <div className="App">
          ...
          <span>{`Question ${currentQuestion + 1} of ${FootballMediumData.length}`}</span>
          {options.map(option => (
            <p
              key={option.id}
         ...
         `}
          ...
          {currentQuestion < FootballMediumData.length - 1 && (
            <button
...
export default FootballMediumQuiz;
