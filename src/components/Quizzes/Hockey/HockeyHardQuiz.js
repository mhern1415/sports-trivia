...
import { HockeyHardData } from "../../QuizData/Hockey/HockeyHardData";
import ProgressBar from 'react-bootstrap/ProgressBar'


class HockeyHardQuiz extends React.Component {
  state = {
    currentQuestion: 0,
    myAnswer: null,
    options: [],
  ...
  loadQuizData = () => {
    this.setState(() => {
      return {
        questions: HockeyHardData[this.state.currentQuestion].question,
        answer: HockeyHardData[this.state.currentQuestion].answer,
        options: HockeyHardData[this.state.currentQuestion].options
      };
    });
  };
  ...
  finishHandler = () => {
    if (this.state.currentQuestion === HockeyHardData.length - 1) {
      this.setState({
        isEnd: true
      });
    }
...
<span>Try Again!</span> </button> 
      ...
      return (
        ...
        <div className="App">
          ...
          <span>{`Question ${currentQuestion + 1} of ${HockeyHardData.length}`}</span>
          {options.map(option => (
            <p
              key={option.id}
         ...
         `}
          ...
          {currentQuestion < HockeyHardData.length - 1 && (
            <button
